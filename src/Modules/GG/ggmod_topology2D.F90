...
module ggmod_topology2D
    ...
    implicit none
    private 
    public :: TopomeshUDT, ConstructTopologicalMesh
    ...
    function ConstructTopologicalMesh(vessel, magneticField, options) result(topomesh)
        ...
        ! Compute extrema
        !================
        call TraceExtrema2D(xe, ye, fe, typee, fieldtracer, &
            magneticField, options%fdonewton)
        
        ! Update the tracers
        !===================
        ! Construct refined grid based on extrema
    ...
    subroutine InitializeTopologicalMeshVertex(tpvert)
        ...
        !===========
        tpvert%ntot = 0
        allocate(tpvert%ID(0), tpvert%x(0), tpvert%y(0), tpvert%F(0))
