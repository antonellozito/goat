subroutine ExtractVesselData(vessel, vesseloptions)
    ...
    ! call Plot2DPolygon(tempx, tempy, nvp+nvs-1-nexcl, '-p')
    !call Plot2DPolygon(vessel%polygonset%polygons(2)%x, &
    !    vessel%polygonset%polygons(2)%y, size(vessel%polygonset%polygons(2)%x), '-p')
